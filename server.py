...
def init_db():
    ...
    with db() as conn:
        conn.execute("""CREATE TABLE IF NOT EXISTS users (user_id INTEGER PRIMARY KEY, username TEXT, first_name TEXT, last_name TEXT, created_at TEXT NOT NULL, last_seen TEXT NOT NULL)""")
        conn.execute("""CREATE TABLE IF NOT EXISTS usage (user_id INTEGER NOT NULL, usage_date TEXT NOT NULL, used INTEGER NOT NULL DEFAULT 0, PRIMARY KEY (user_id, usage_date))""")
        conn.execute("""CREATE TABLE IF NOT EXISTS paid_plans (user_id INTEGER PRIMARY KEY, daily_limit INTEGER NOT NULL, expires_at TEXT NOT NULL)""")
        conn.commit()

def save_user(chat):
    now = datetime.now().isoformat()
    with db() as conn:
        conn.execute("""INSERT INTO users(user_id, username, first_name, last_name, created_at, last_seen) VALUES(?,?,?,?,?,?) ON CONFLICT(user_id) DO UPDATE SET username=excluded.username, first_name=excluded.first_name, last_name=excluded.last_name, last_seen=excluded.last_seen""", (chat.id, chat.username, chat.first_name, chat.last_name, now, now))
        conn.commit()
...
def get_plan(user_id: int):
    with db() as conn:
        ...
        conn.execute("DELETE FROM paid_plans WHERE user_id=?", (user_id,)); conn.commit()
    return None

def current_limit(user_id: int) -> int:
    plan = get_plan(user_id)
    return int(plan["daily_limit"]) if plan else FREE_DAILY_LIMIT
...
def check_and_use_request(user_id: int) -> tuple[bool, int, int]:
    ...
    with db() as conn:
        ...
        used += 1
        conn.execute("INSERT INTO usage(user_id, usage_date, used) VALUES(?,?,?) ON CONFLICT(user_id, usage_date) DO UPDATE SET used=excluded.used", (user_id, today, used)); conn.commit()
    return True, limit-used, limit

def menu():
    return ReplyKeyboardMarkup([
        [KeyboardButton("🏟 Сделать анализ"), KeyboardButton("💳 Тарифы")],
        [KeyboardButton("👤 Мой профиль"), KeyboardButton("📊 Мои запросы")],
        [KeyboardButton("ℹ️ Как это работает"), KeyboardButton("👨‍💻 Поддержка")],
...
async def profile_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not chat: return
    save_user(chat)
    plan = get_plan(chat.id)
    limit = int(plan["daily_limit"]) if plan else FREE_DAILY_LIMIT
    today = datetime.now().date().isoformat()
    with db() as conn:
        row = conn.execute("SELECT used FROM usage WHERE user_id=? AND usage_date=?", (chat.id, today)).fetchone()
    used = int(row["used"]) if row else 0
    if plan:
...
async def give_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with db() as conn:
        ...
        conn.execute("DELETE FROM usage WHERE user_id=? AND usage_date=?", (user_id,datetime.now().date().isoformat())); conn.commit()
...
async def relay_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🏟 Сделать анализ":
        await message.reply_text("🏟 Отлично! Отправьте матч или скриншот линии сюда 👇", reply_markup=menu()); return
    if chat.id not in welcomed_users:
        welcomed_users.add(chat.id); await message.reply_text(WELCOME, reply_markup=menu())
    ...
    try:
        ...
        log.exception("failed to relay user message"); await message.reply_text("⚠️ Не удалось передать запрос. Попробуй ещё раз.", reply_markup=menu())
...
app=FastAPI(title="Sport Risk Analyst Pro",version="0.9.0",lifespan=lifespan)
@app.get("/")
